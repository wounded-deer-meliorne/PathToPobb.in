...
(function () {
  ...
  function findPobCode() {
    ...
    for (const sel of [
      ...
      for (const el of els) {
        ...
        if (code) {
          console.log('[maxroll-to-pobb] Found via selector:', sel);
          return code;
        }
      }
    }
    ...
    for (const el of document.querySelectorAll('textarea')) {
      const code = getCodeFromElement(el);
      if (code) {
        console.log('[maxroll-to-pobb] Found via textarea scan');
        return code;
      }
    }
  ...
  function dispatch(code) {
    ...
    console.log('[maxroll-to-pobb] Dispatching POB code to background...');
  ...
  const poll = setInterval(() => {
    ...
    if (++attempts >= MAX_ATTEMPTS) {
      ...
      observer.disconnect();
      overlay.remove();
      console.warn('[maxroll-to-pobb] Gave up after 30s. textarea.poe-textarea not found or value was empty.');
    }
  }, 500);
