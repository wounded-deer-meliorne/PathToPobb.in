...
(function () {
  ...
  chrome.storage.local.get('pendingPobCode', ({ pendingPobCode }) => {
    if (!pendingPobCode) {
      console.log('[maxroll-to-pobb] No pending POB code found in storage.');
      return;
    }

    console.log('[maxroll-to-pobb] POB code found in storage, length:', pendingPobCode.length);
  ...
  function injectWhenReady(code, attempt = 0) {
    ...
    if (textarea) {
      console.log('[maxroll-to-pobb] Textarea found, injecting...');
    ...
    if (attempt < MAX_ATTEMPTS) {
      ...
      console.warn('[maxroll-to-pobb] Textarea never appeared after 12s.');
    }
  }
  ...
  function inject(textarea, code) {
    ...
    console.log("[maxroll-to-pobb] Injected. Value length:", textarea.value.length);
    ...
    setTimeout(() => {
      ...
      if (createBtn) {
        console.log("[maxroll-to-pobb] Clicking Create button...");
        createBtn.click();
      } else {
        console.warn("[maxroll-to-pobb] Create button not found.");
      }
    }, 500);
  }
